...
class SH0ESLikelihood(Likelihood):
    ...
    def __call__(self, **params) -> float:
        """Allow direct evaluation from parameter dict without a cosmology object."""
        if "H0" not in params:
            raise ValueError("SH0ES likelihood requires parameter 'H0'.")
        H0 = jnp.asarray(params["H0"], dtype=jnp.float32)
        diff = (H0 - self.data.H0_mean) / self.data.H0_sigma
        return -0.5 * diff**2 - jnp.log(self.data.H0_sigma * jnp.sqrt(2.0 * jnp.pi))
