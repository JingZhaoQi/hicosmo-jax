...
class MCMCState:
    """
    MCMC state for checkpointing and recovery (simplified).

    Essential fields only - removed environment collection, time estimation, etc.
    """
    ...
    random_state: Optional[Dict] = None  # For compatibility
...
class CheckpointManager:
    ...
    def save_checkpoint(
        self, state: MCMCState, run_name: str, force: bool = False
    ) -> Optional[Path]:
        """
        ...
        # Use step-based filename to support list/resume/cleanup
        step = int(state.current_step or 0)
        filename = f"{run_name}_step_{step}.h5"
        ...
        try:
            ...
            # Update "latest" alias for convenience (used by plotters/tools)
            self._update_latest_alias(filepath, run_name)
    ...
    def _update_latest_alias(self, filepath: Path, run_name: str) -> None:
        """Maintain a latest checkpoint alias at {run_name}.h5."""
        import os
        import shutil

        latest_path = self.checkpoint_dir / f"{run_name}.h5"
        try:
            ...
            try:
                ...
                warnings.warn(f"Could not update latest checkpoint alias: {e}")
    ...
    def _save_hdf5_checkpoint(self, state: MCMCState, filepath: Path):
        ...
        with h5py.File(filepath, "w") as f:
            ...
            # Save LaTeX labels for Plotter compatibility
            # Labels come from parameter_config (filled by Parameter._set_defaults())
            if state.samples:
                labels_group = meta_group.create_group("labels")
                config_labels = (
                    state.parameter_config.get_labels()
                    if state.parameter_config
                    else {}
                )
                for param_name in state.samples.keys():
                    label = config_labels.get(param_name, param_name)
                    labels_group.attrs[param_name] = label
            ...
            if state.samples:
                ...
                for param_name, samples in state.samples.items():
                    samples_group.create_dataset(
                        param_name, data=samples, compression=compression
                    )
            ...
            for state_name, state_data in [
                ...
                if state_data:
                    ...
                    try:
                        if state_name == "mcmc_state":
                            raise TypeError("Force pickle for mcmc_state")
                        ...
                        state_group.create_dataset(
                            "state_pickle", data=np.void(state_pickle)
                        )
            ...
            try:
                with h5py.File(filepath, "r") as f:
                    ...
                    checkpoint_info = {
                        ...
                        "completion_percentage": progress.attrs[
...
class ResumeManager:
    ...
    def _check_parameter_compatibility(
        self, saved_config: ParameterConfig, current_config: ParameterConfig
    ) -> List[str]:
        ...
        if missing_params:
            issues.append(
                f"CRITICAL: Missing parameters in current config: {missing_params}"
            )
        ...
        if extra_params:
            issues.append(
                f"WARNING: Extra parameters in current config: {extra_params}"
            )
        ...
        for param_name in common_params:
            ...
            if saved_param.prior != current_param.prior:
                if self.strict_validation:
                    issues.append(
                        f"CRITICAL: Prior changed for {param_name}: "
    ...
    def _check_likelihood_compatibility(
        self, saved_info: Dict[str, Any], current_likelihood: Callable
    ) -> List[str]:
        ...
        if saved_hash and current_hash and saved_hash != current_hash:
            if self.strict_validation:
                ...
                issues.append(
                    "WARNING: Likelihood function source code may have changed"
                )

        return issues

    def _check_data_compatibility(
        self, saved_info: Dict[str, Any], current_data: Dict[str, Any]
    ) -> List[str]:
        """Check data compatibility."""
        issues = []
