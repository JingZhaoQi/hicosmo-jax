...
class ParameterConfig:
    ...
    def get_labels(self) -> Dict[str, str]:
        ...
        """
        return {
            name: param.latex_label or name
            for name, param in self.parameters.items()
        }

    def get_reference_values(self) -> Dict[str, float]:
        """Get reference values for all parameters."""
        return {name: param.value for name, param in self.parameters.items() if param.value is not None}

    def get_numpyro_priors(self) -> Dict[str, Any]:
        """Get NumPyro distribution objects for all free parameters."""
        return {name: param.to_numpyro_dist() for name, param in self.parameters.items() if param.free}
    ...
    def from_simple_list(cls, param_list: List[List], **mcmc_kwargs) -> 'ParameterConfig':
        ...
        **mcmc_kwargs
        ...
        for param_def in param_list:
            ...
            parameters[name] = Parameter.from_tuple(name, (initial, min_val, max_val, latex))
