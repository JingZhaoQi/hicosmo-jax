...
def _require_getdist() -> None:
    if not HAS_GETDIST:
        raise ImportError("GetDist is required for HIcosmo plotting. Install with: pip install getdist")
...
def _load_chain_simple(filename: str):
    ...
    if file_path.suffix == '.npy':
        ...
        if '_param_names' in npz_file.files:
            ...
            labels_arr = npz_file['_labels'].tolist() if '_labels' in npz_file.files else param_names
            ...
            metadata = {
                ...
                'ranges': dict(zip(param_names, [tuple(r) for r in ranges_arr])) if ranges_arr is not None else {}
            }
                ...
                if 'metadata' in f:
                    ...
                    if 'ranges' in meta_group:
                        ...
                        for key in ranges_group.keys():
                            ...
                            metadata['ranges'][key] = (float(bounds[0]), float(bounds[1]))
...
def _normalize_range(bounds: Union[Tuple[float, float], List[float]]) -> Tuple[float, float]:
    lo, hi = bounds
...
def _merge_range_dicts(*range_dicts: Optional[Dict[str, Tuple[float, float]]]) -> Dict[str, Tuple[float, float]]:
...
def _coerce_ranges_arg(ranges: Optional[Union[Dict[str, Tuple[float, float]], List[Any], Tuple[Any, ...]]]) -> Optional[Dict[str, Tuple[float, float]]]:
    if ranges is None:
        return None
...
def _prepare_getdist_samples(data, params=None, labels=None, ranges=None, param_ranges=None, **kwargs) -> MCSamples:
    """
    ...
    **kwargs
    ...
    if isinstance(data, MCSamples):
        ...
        resolved = _resolve_ranges(param_names, data.samples, explicit_ranges=merged_ranges)
        if resolved:
            copied = data.copy()
    ...
    merged_ranges = _merge_range_dicts(file_ranges, _coerce_ranges_arg(ranges), param_ranges)
    resolved_ranges = _resolve_ranges(param_names, samples, explicit_ranges=merged_ranges)
    ...
    with _suppress_stdout():  # Suppress getdist "Removed no burn in" message
        return MCSamples(samples=samples, names=param_names, labels=plot_labels,
                         ranges=resolved_ranges, ignore_rows=ignore_rows, **kwargs)
