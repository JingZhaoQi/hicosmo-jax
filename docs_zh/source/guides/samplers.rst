...
.. code-block:: python
   ...
   samples = mcmc.run(
       ...
       num_chains=4,          # 并行链数
       num_warmup=1000,       # 每条链的预热步数
       progress_bar=True      # 显示进度条
   )

**两个计数参数的语义不同，请注意区分**：

- ``num_samples`` 是所有链的**总样本数**：4 链 4000 样本意味着每链 1000 样本。
- ``num_warmup`` 是**每条链**的预热步数：预热是每条链独立完成的质量矩阵
  与步长自适应，统计上不能在链间摊薄。未指定时默认取
  ``max(200, min(1000, 每链样本数))``。

.. note::

   ``progress_bar`` 未显式指定时，仅在交互终端（TTY）显示；输出重定向到
   文件或在批处理环境运行时自动关闭。NumPyro 的进度条每步触发一次
   host 回调，对毫秒级似然有 5–20% 的开销，关闭它还能避免日志刷屏。
